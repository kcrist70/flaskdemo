    ...
    def insert_redis(self,day_data,mon_data):
        with self.redis.pipeline() as p:
            for data_dict in day_data['aggregations']['group_by_robot']['buckets']:
                p.hset(data_dict['key'], data_dict['key'] + "_pv_" + time.strftime("%Y-%m-%d",time.localtime()), data_dict['doc_count'])
            for data_dict in mon_data['aggregations']['group_by_robot']['buckets']:
                p.hset(data_dict['key'], data_dict['key'] + "_pv_" + time.strftime("%Y-%m",time.localtime()), data_dict['doc_count'])
                p.hset(data_dict['key'], data_dict['key'] + "_uv_" + time.strftime("%Y-%m",time.localtime()), data_dict['distinct_by_uid']['value'])
            p.execute()
